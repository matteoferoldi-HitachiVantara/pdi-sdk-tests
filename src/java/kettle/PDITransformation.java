...
public class PDITransformation extends PDIBaseObject {
  ...
  public boolean isSavedInRepo( Repo repo, String path ) {
    if ( repo.isExist( this.getName(), path, RepositoryObjectType.JOB ) ) {
      LOGGER.info( "Transformation " + name + " was saved to " + path + " in the repository." );
      return true;
    } else {
      LOGGER.info( "Transformation " + name + " was not saved to " + path + " in the repository." );
      return false;
    }
  }

...
public class Repo {
  ...
  public RepositoryMeta findRepository( String repositoryName ) {
    ...
    if ( repositoryMeta == null ) {
      LOGGER.info(
        "Repository '" + repositoryName + "' not found in your " + Const.getKettleUserRepositoriesFile() + " file" );
    }
  ...
  public RepositoryDirectoryInterface moveDir( RepositoryDirectoryInterface dir, String path ) {
    ...
    if ( !dir.getParent().getPath().equals( path ) ) {
      throw new RuntimeException(
          "New directory: " + dir.getName() + " is in incorrect place " + dir.getParent().getPath() );
    }
  ...
  public List<RepositoryElementMetaInterface> getDeletedJobs( String path ) {
    return getDeletedObjects( path, RepositoryObjectType.JOB );
  }

  public List<RepositoryElementMetaInterface> getDeletedTransformations( String path ) {
    return getDeletedObjects( path, RepositoryObjectType.TRANSFORMATION );
  }

  private List<RepositoryElementMetaInterface> getDeletedObjects( String path, RepositoryObjectType type ) {
    RepositoryDirectoryInterface dir = findDir( path );
    ...
    if ( objects != null ) {
      LOGGER.info( "The folder " + dir.getName() + " contains the following Deleted objects:" );
      ...
      LOGGER.warn( "None of deleted objects found." );
    }
